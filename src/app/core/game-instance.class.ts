...
export class GameInstance {
  ...
  // public ths: TriggerHelperService; // don't instantiate
  // public gameNumber: number; // superfluous
  public id: string;
  public players: Array<Player>;
  public format: string;
  // public effects: Array<Modifier>; // should not need this - it ought to be in battlefield.class
  // public objects: Array<Permanent>; // should not need this - it ought to be in battlefield.class
  ...
  // start the game
  public start(): void {
    console.log('called gameInstance.start() - did we successfully start?');
    // assume all necessary variables have been initialized
    _.each(this.players, (player: Player) => {
      for (let i = 0; i < 7; i++) {
        player.drawCard();
      }
    });
    Logger.gLog('each player has now drawn 7 cards');
  }
  ...
  public nextPhase(): void {
    ...
    if (this.phase.name() === 'cleanup') {
      this.turn.end();
      Turn.turnCount++;
      this.turn = new Turn(this.activePlayer.opponent, this.activePlayer);
      this.turns.push(this.turn);
      this.phase.advancePhase();
    }
    if (this.phase.name() === 'untap') {
      _.each(this.activePlayer.controls, (obj: any) => {
        obj.untap();
      });
    }
  }
  ...
  // vvv should be a method of another class (its own controller or service probably)
  ...
  // vvv implement in Battlefield class
